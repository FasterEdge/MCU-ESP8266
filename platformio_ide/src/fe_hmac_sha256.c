// fe_hmac_sha256.c — HMAC-SHA256 纯 C 实现（零依赖）
...
static void transform(fe_sha256_ctx *ctx, const uint8_t *data) {
...
void fe_sha256_final(fe_sha256_ctx *ctx, uint8_t hash[32]) {
    ...
    for (int j = 0; j < 8; j++)
        ctx->data[56 + j] = (uint8_t)(bitlen >> (56 - j * 8));
    transform(ctx, ctx->data);
    for (int j = 0; j < 8; j++) {
